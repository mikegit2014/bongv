...
class Basememcache{
    ...
    private $MEMCACHE_EXPIRATION;
    private $MEMCACHE_PREFIX;
    private $MEMCACHE_COMPRESSION;
     
     
    public function __construct($host='127.0.0.1',$port=11211,$expire=0,$prefix='bongv_',$compression=false)
    {
        $this->MEMCACHE_HOST = $host;
        $this->MEMCACHE_PORT = $port;
        $this->MEMCACHE_EXPIRATION = $expire;
        $this->MEMCACHE_PREFIX = $prefix;
        $this->MEMCACHE_COMPRESSION = $compression;
    ...
    **/
    private function auto_connect()
    {
        $configServer = 
            array(
                'host' => $this->MEMCACHE_HOST,
                'port' => $this->MEMCACHE_PORT,
                'weight' => 1,
            );
        if(!$this->add_server($configServer)){
            echo 'ERROR: Could not connect to the server named '.$this->MEMCACHE_HOST;
        }else{
        }
    }
    ...
    **/
    public function add($key = NULL, $value = NULL, $expiration = 0)
    {
        if(is_null($expiration)){
            $expiration = $this->MEMCACHE_EXPIRATION;
        }
        if(is_array($key))
        {
            foreach($key as $multi){
                if(!isset($multi['expiration']) || $multi['expiration'] == ''){
                    $multi['expiration'] = $this->MEMCACHE_EXPIRATION;
                }
            ...
            switch($this->client_type){
                case 'Memcache':
                    $add_status = $this->m->add($this->key_name($key), $value, $this->MEMCACHE_COMPRESSION, $expiration);
                    break;
    ...
    **/
    public function set($key = NULL, $value = NULL, $expiration = NULL)
    {
        ...
        if(is_array($key))
        {
            ...
            switch($this->client_type){
                case 'Memcache':
                    $add_status = $this->m->set($this->key_name($key), $value, $this->MEMCACHE_COMPRESSION, $expiration);
                    break;
                case 'Memcached':
    ...
    **/
    public function replace($key = NULL, $value = NULL, $expiration = NULL)
    {
        ...
        if(is_array($key)){
            ...
            switch($this->client_type){
                case 'Memcache':
                    $replace_status = $this->m->replace($this->key_name($key), $value, $this->MEMCACHE_COMPRESSION, $expiration);
                    break;
                case 'Memcached':
