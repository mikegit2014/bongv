<?php
    ...
    $image->open('test.jpg');
    $image->cropimage(200, 120);
    $image->save_to('test1.jpg');
    ...
    $image->save_to('test_small.jpg');

    3.添加图片水印：

    $image = new \lib_image_imagick();
    $image->open('test.jpg');
    $image->add_watermark('test_small.jpg');
    $image->save_to('test_water.jpg');

    4.添加文字水印：

    $image = new \lib_image_imagick();
    $image->open('test.jpg');
    $image->add_text('http://www.phpddt.com',50,50,0,array('font'=>'ARIAL.TTF','font_size'=>14,'fill_color'=>'white','under_color'=>'black'));
    $image->output();
    5.改变图片尺寸：

    $image = new \lib_image_imagick();
    $image->open('test.jpg');
    $image->resize_to(800,800);
    $image->output();*/
